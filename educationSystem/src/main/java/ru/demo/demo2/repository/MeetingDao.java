...
public class MeetingDao extends BaseDao<Meeting> {
    ...
    public List<Meeting> findByPairId(Integer pairId) {
        try (Session s = HibernateSession.getSessionFactory().openSession()) {
            return s.createQuery("FROM Meeting WHERE pair.id = :id ORDER BY datetime DESC", Meeting.class)
    ...
    public Double getAverageMentorRating(Integer pairId) {
        try (Session s = HibernateSession.getSessionFactory().openSession()) {
            Double r = s
                    .createQuery("SELECT AVG(mentorRating) FROM Meeting WHERE pair.id = :id AND mentorRating IS NOT NULL",
                            Double.class)
                    .setParameter("id", pairId).uniqueResult();
            return r != null ? r : 0.0;
        }
    }
    ...
    public Double getAverageMenteeRating(Integer pairId) {
        try (Session s = HibernateSession.getSessionFactory().openSession()) {
            Double r = s
                    .createQuery("SELECT AVG(menteeRating) FROM Meeting WHERE pair.id = :id AND menteeRating IS NOT NULL",
                            Double.class)
                    .setParameter("id", pairId).uniqueResult();
            return r != null ? r : 0.0;
        }
